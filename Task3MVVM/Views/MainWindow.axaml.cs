...
public partial class MainWindow : Window
{
    ...
    private void SaveResultsButton(object? sender, RoutedEventArgs e)
    {
        if (viewModel.IsTestCompleted() == true)
            Model.SaveTest("/Users/michael/Documents/University (original)/2 course/casd/casd-labs/Task3/Task3MVVM/Task3MVVM/Models/arrays.txt");
        else
        {
            viewModel.Alert();
            DataContext = viewModel;
    ...
    private void DataType_OnSelectionChangedSelection_OnSelectionChanged(object? sender, SelectionChangedEventArgs e)
    {
        var comboBox = sender as ComboBox;
        if (comboBox != null & comboBox.SelectedIndex != null)
        {
            if (viewModel != null)
            {
                viewModel.SelectedDataIndx = comboBox.SelectedIndex;
                viewModel.Alert();
                DataContext = viewModel;
            }
        }
